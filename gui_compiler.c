...
#define ID_BTN_PROCMAN 113

// Global Variables
HWND hGccPath, hFileCombo, hBtnBrowse, hBtnEdit, hBtnCompile, hBtnCopy, hOutput, hBtnRun, hArgs, hBtnRestore, hBtnBackup, hBtnPaste, hBtnProcMan;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hBtnProcMan= CreateWindow("BUTTON", "ProcMan", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 0, 0, hwnd, (HMENU)ID_BTN_PROCMAN, NULL, NULL);
            ...
            SendMessage(hBtnProcMan, WM_SETFONT, (WPARAM)hLargeFont, TRUE);
        ...
        case WM_SIZE: {
            ...
            // Row 4: Action Buttons (8 Buttons)
            int numBtns = 8;
            ...
            MoveWindow(hBtnProcMan, pad*4 + itemW*3,       btnY, itemW, btnH, TRUE);
            MoveWindow(hBtnRestore, pad*5 + itemW*4,       btnY, itemW, btnH, TRUE);
            MoveWindow(hBtnBackup,  pad*6 + itemW*5,       btnY, itemW, btnH, TRUE);
            MoveWindow(hBtnPaste,   pad*7 + itemW*6,       btnY, itemW, btnH, TRUE);
            MoveWindow(hBtnCopy,    pad*8 + itemW*7,       btnY, width - (pad*9) - (itemW*7), btnH, TRUE);
        ...
        case WM_COMMAND: {
            ...
            else if (LOWORD(wParam) == ID_BTN_PROCMAN) {
                char filepath[MAX_PATH];
                GetWindowText(hFileCombo, filepath, MAX_PATH);
                if (strlen(filepath) > 0) {
                    SaveSettings();

                    // Determine the directory where gui_compiler.exe is located
                    char procManDir[MAX_PATH];
                    GetModuleFileName(NULL, procManDir, MAX_PATH);
                    char *lastSlash = strrchr(procManDir, '\\');
                    if (lastSlash) *lastSlash = '\0'; // Isolate just the directory path

                    // Construct full path to ProcManager.exe
                    char procManExe[MAX_PATH];
                    snprintf(procManExe, sizeof(procManExe), "%s\\ProcManager.exe", procManDir);

                    // Wrap the target file path in quotes to handle spaces
                    char argsBuf[MAX_PATH + 32];
                    snprintf(argsBuf, sizeof(argsBuf), "\"%s\"", filepath);

                    // Execute ProcManager.exe explicitly setting its working directory to its own location
                    HINSTANCE hInst = ShellExecute(NULL, "open", procManExe, argsBuf, procManDir, SW_SHOWNORMAL);
                    if ((INT_PTR)hInst <= 32) {
                        SetWindowText(hOutput, "Error: Failed to run ProcManager.exe. Ensure it is in the same directory as gui_compiler.exe.");
                    } else {
                        SetWindowText(hOutput, "ProcManager launched successfully.");
                    }
                }
            }
